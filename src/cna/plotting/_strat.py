...
def violinplot(data, stratification, key="coef", ax=None, cmap="seismic", **kwargs):
    ...
    violins: dict[str, Any] = ax.violinplot(
        [data.obs.loc[data.obs[stratification] == v][key] for v in bins], np.arange(len(bins)), **kwargs_
    )
    ...
    for violin in violins["bodies"]:
        ...
        ax.add_patch(patch)
        _ = ax.imshow(
            imgArr, origin="lower", extent=[xmin, xmax, ymin, ymax], aspect="auto", cmap=cmap, clip_path=patch
        )
