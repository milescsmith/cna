...
def violinplot(data, res, stratification, ax=None, cmap="seismic", **kwargs):
    ...
    violins = ax.violinplot([res.ncorrs[data.obs[stratification] == v] for v in bins], np.arange(len(bins)), **kwargs_)
    ...
    for violin in violins["bodies"]:
        ...
        ax.add_patch(patch)
        ax.imshow(imgArr, origin="lower", extent=[xmin, xmax, ymin, ymax], aspect="auto", cmap=cmap, clip_path=patch)
