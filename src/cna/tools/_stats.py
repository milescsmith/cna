...
def conditional_permutation(B, Y, num):
    ...
    ix = np.concatenate([bi[np.argsort(rng.standard_normal(size=(len(bi), num)), axis=0)] for bi in batchind])
...
def grouplevel_permutation(G, Y, num):
    ...
    if (Yg[Gind] != Y).any():
        msg = "ERROR: the value of Y is not identical within each group of samples"
        raise ValueError(msg)
    rng = np.random.default_rng()
    ix = np.argsort(rng.standard_normal(size=(len(Yg), num)), axis=0)
    return Yg[ix][Gind]
...
def empirical_fdrs(z, znull, thresholds):
    ...
    if znull.shape[0] != len(z):
        msg = f"ERROR: znull is shape {znull.shape} and z is shape {z.shape}"
        raise ValueError(msg)
