...
def svd_nam(NAM):
    ...
    U, svs, _ = np.linalg.svd(NAM.dot(NAM.T))
    V = NAM.T.dot(U) / np.sqrt(svs)

    return (
        pd.DataFrame(U, index=NAM.index, columns=pd.Index([f"PC{i!s}" for i in range(1, len(U.T) + 1)])),
        pd.Series(svs, index=["PC" + str(i) for i in range(1, len(U.T) + 1)]),
        pd.DataFrame(V.values, index=NAM.columns, columns=pd.Index([f"PC{i!s}" for i in range(1, len(U.T) + 1)])),
    )
